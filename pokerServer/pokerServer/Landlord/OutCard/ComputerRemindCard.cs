...
namespace pokerServer.Landlord.OutCard {
    class ComputerRemindCard {
        ...
        private static List<Card>[] nextRemindCard = new List<Card>[3];
        //机器人智能出牌，包含各种情况
        ...
        private static List<Card> farmerAIOutCard(List<Card>[] peoplesCard, int whosland, int whoOut, int nowPerson, List<Card> preCard) {
            ...
            bool canNextPeopleOutAllCard = false;
            int i, j;
            //循环后面的农民，遇到地主就停
            for (i = nowPerson, j = 0; i != whosland; i = (i + 1) % 4, j++) {
                ...
                //如果有人能出完（跳出）
                if (nextRemindCard[j].Count == peoplesCard[i].Count)
                    break;
            }

            List<Card> curOutCards = new List<Card>();
            //如果自己能出完或者自己什么牌都出不了（或者自己出完这次牌后，下一次能一次出完）
            if (i == nowPerson || nextRemindCard[0].Count == 0
            ...
            //如果没有人能出完
                ...
                //农民打农民或农民打地主
                    ...
                    if (whoOut != whosland) {
                        ...
                        bool canOutAllCard = afterThisOut_CanOutAllCard(peoplesCard[whoOut], preCard);
                        if (canOutAllCard == true) {
                            canNextPeopleOutAllCard = true;
                        }
                        //如果用2，大小王打其他农民,前面已经判断，不能一次性出完
                        else if (nextRemindCard[0][0].cardSize >= 15) {
                            canNextPeopleOutAllCard = true;
                        }
                    }
        ...
        private static List<Card> farmerCrushCard(List<Card>[] people, int whosland, int whoOut, int nowPerson, List<Card> prevCard) {
            ...
            for (i = nowPerson, j = 0; i != whosland; i = (i + 1) % 4, j++) {
                //如果该家出的是普通牌
                if (OutCardStyle.isBomb(nextRemindCard[j]) == 0 && nextRemindCard[j].Count != 0)
                    break;
            }
        ...
        private static List<Card> robortSingleDefend(List<Card> deck, List<Card> prevCard) {
            ...
            for (i = deck.Count - 1; i >= 0; i--) {
                if (deck[i].cardSize < 11 || deck[i].sameCardNum > 3 || deck[i].cardSize <= prevCard[0].cardSize)
                    continue;
                resCards.Add(deck[i]);
                return resCards;
            }
